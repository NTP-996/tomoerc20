        ...
        <div ref="tokenTableContainer" class="token-table-container">
          ...
          <table v-show="localTokens.length > 0 && receivedTokens">
            <tr v-for="(token, index) in localTokens" :key="token.name + index">
              <td>
                <img src="./tokenIcon/favicon-32x32.png">
                {{ token.name }}
              </td>
              <td v-if="token.balance === 'Load'" class="load-token">{{ token.balance }}</td>
...
export default {
  ...
  methods: {
    ...
    async assignTokens(arr, query) {
      ...
      this.localTokens.forEach((e, i) => this.getSpecificTokenBalance(e, i));
      // this.getSpecificTokenBalance();
    }
  }
};
</script>
